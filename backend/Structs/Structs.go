...
type MRB struct {
	...
	CreationDate [16]byte     // 16 bytes "2021-08-08 15:04"
	Signature    int32        // 4 bytes
	Fit          [1]byte      // 1 byte
	Partitions   [4]Partition //cantidad de particiones que admite el disco
}

func PrintMBR(data MRB) {
	fmt.Println("====== MBR ======")
	fmt.Println(fmt.Sprintf("Size: %d, CreationDate: %s, Signature: %d, fit: %s", data.MbrSize, string(data.CreationDate[:]), data.Signature, string(data.Fit[:])))
...
func PrintPartition(data Partition) {
	fmt.Println(fmt.Sprintf("status: %s, type: %s, fit: %s, start: %d, size: %d, name: %s, correlative: %d, id: %s",
		string(data.Status[:]),
		string(data.Type[:]),
		string(data.Fit[:]),
		data.Start,
...
func PrintEBR(data EBR) {
	fmt.Println("====== EBR ======")
	fmt.Println(fmt.Sprintf("Mount: %c, fit: %c, start: %d, size: %d, next: %d, name: %s",
		data.PartMount,
		data.PartFit,
		data.PartStart,
		data.PartSize,
...
type Superblock struct {
	...
	S_first_ino         int32    // Primer inodo libre (dirección del inodo)
