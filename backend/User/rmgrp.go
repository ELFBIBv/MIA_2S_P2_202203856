...
func Rmgrp(name string, id string) error {
	...
	//vamos a comprobar de que no hayan usuarios afiliados al grupo a borrar
	for _, line := range lines {
		palabras := strings.Split(line, ",")
		if palabras[0] != "0" && palabras[1] == "U" && palabras[2] == name {
			fmt.Println("Error: El grupo tiene usuarios afiliados")
			return errors.New("el grupo tiene usuarios afiliados")
		}
	}
	...
	for _, line := range lines {
		...
		if palabras[0] == "0" && palabras[1] == "G" && palabras[2] == name {
			fmt.Println("Error: El grupo ya está desactivado")
			return errors.New("el grupo ya está desactivado")
		}
	}

	found := false
	//revisamos que el grupo exista
	for _, line := range lines {
		palabras := strings.Split(line, ",")
		if strings.Contains(palabras[1], "G") && strings.Contains(palabras[2], name) {
			found = true
		}
	}
	...
	for _, line := range lines {
		palabras := strings.Split(line, ",")
		if palabras[1] == "U" || palabras[2] != name {
